...
<figure>
  ![SP initiated SSO](@site/docs/sso/assets/single-sign-on/testing-sso/with-idp-simulator/1.1-test-organization.png)
  <figcaption>
    The Test Organization is located at Workspace → Development → Organizations → Test Organization.
  </figcaption>
</figure>

Here is a code snippet that demonstrates generating an Authorization URL using an Organization ID, as outlined in Step 2 of the sequence diagram.

<CodeWithHeader title="Nodejs">

```js
const organisation = 'org_01J1H4ZRQMJ7FK82P5A39JBDCQ';
const options = {};
...
The user will be redirected to the Identity Provider (IdP) Simulator, where they can select a scenario to simulate as a user.

<figure>
  ![IdP Simulator](@site/docs/sso/assets/single-sign-on/testing-sso/with-idp-simulator/2.png)
  <figcaption>
    IdP Simulator
  </figcaption>
</figure>

Select "User Login via SSO" to view a form. This form is used to simulate the Identity Provider (IdP) passing the relevant details that correspond to a user signing into your application.

<figure>
  ![IdP Simulator](@site/docs/sso/assets/single-sign-on/testing-sso/with-idp-simulator/3.png)
  <figcaption>
    User Login via SSO
  </figcaption>
</figure>

Enter the user details and simulate the login. You will be redirected to your application with the same details, indicating that your simulation was successful.
...
<figure>
  ![IdP initiated SSO](@site/docs/sso/assets/single-sign-on/testing-sso/with-idp-simulator/2.png)
  <figcaption>
    IdP initiated SSO
  </figcaption>
</figure>

Implement a condition in your callback handler to identify whether the callback from Scalekit is IdP-initiated or not to handle this scenario.
...
if (idp_initiated_login) {
    ...
    // 3. Generate and redirect the user to Authorization URL
  }
```
</CodeWithHeader>
...
Learn more about [implementing IdP initiated SSO](sso/guides/setup-sso/implement-idp-initiated-sso).

:::note
For simulation purposes in the IdP simulator, you can select and simulate IdP-initiated SSO to test your application. In an ideal scenario, users would choose your app from their IdP and begin the sign-in process into your app.
:::
...
<figure>
  ![Error Scenario](@site/docs/sso/assets/single-sign-on/testing-sso/with-idp-simulator/5.png)
    <figcaption>
