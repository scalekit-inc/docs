...
1. When Scalekit receives an IdP-initiated SSO request from one of your customers, it pings your configured default <SimpleCode>redirect_uri</SimpleCode> with the <SimpleCode> idp_initiated_login</SimpleCode> in it's request parameter with an encoded JWT token.
...
2. The following is a JSON representation of the information that is sent to your registered Redirect URL.
...
3. Utilize the provided information to generate and redirect the user to the Authorization URL. The method <SimpleCode>scalekit.getIdpInitiatedLoginClaims(..)</SimpleCode> automatically decodes the data to extract the <SimpleCode>organization_id</SimpleCode>, <SimpleCode>connection_id</SimpleCode>, and <SimpleCode>login_hint</SimpleCode>. Any one of these properties can be used to successfully generate the Authorization URL.
...
// 5. Continue to create a Session
```
...
public RedirectView callbackHandler(
  ...
  try {
    ...
    AuthenticationResponse res =
        scalekit.authentication().authenticateWithCode(
            code, redirectUrl, new AuthenticationOptions());
