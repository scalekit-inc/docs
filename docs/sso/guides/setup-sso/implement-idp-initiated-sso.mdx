...
```sh
https://b2b-app.dev/default-redirect-uri?idp_initiated_login=<encoded_jwt_token>
```
...
The <SimpleCode>redirect_uri</SimpleCode> is callback with all the information (now shown in JWT) sent as query parameters. Additionally, It also adds the JWT of same information with <SimpleCode>idp_initated_login</SimpleCode> parameter.

<CodeWithHeader>

```sh
http://b2b-app.com/callback?
