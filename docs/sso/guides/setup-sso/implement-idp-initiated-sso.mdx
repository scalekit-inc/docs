---
title: 'Implement IdP-Initiated SSO with Scalekit | Secure Authentication Guide'
description: 'Learn how to implement Identity Provider-Initiated Single Sign-On (SSO) using Scalekit, converting IdP-initiated requests into secure SP-initiated flows.'
keywords:
  [
...
if (!string.IsNullOrEmpty(idpInitiatedLogin))
{
  try
  {
    ...
    // 3. Generate Authorization URL
    string url = scalekitClient.GetAuthorizationUrl(redirectUrl, options);
    return Redirect(url);
  }
  catch (Exception ex)
  {
