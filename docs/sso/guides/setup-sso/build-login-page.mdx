...
In this strategy, instead of a single login page at <SimpleCode>https:<span></span>//app.yoursaas.com/login</SimpleCode>, you serve different login pages for each tenant. For example, <SimpleCode>https://<span></span>tenant1.yoursaas.com/login</SimpleCode>, <SimpleCode>https://<span></span>tenant2.yoursaas.com/login</SimpleCode>. Depending on the tenant URL, you would show only the respective authentication methods applicable to that tenant, thus optimizing the user experience further.
