...
The generated link is valid for 7 days but can be revoked at any time from the dashboard for security purposes.You can share the link through communication channels such as email, Slack, or other preferred methods.
...
When the Admin Portal is embedded in your application via an iframe, it can emit events to the parent window. You can capture these events using the <SimpleCode>window.onmessage</SimpleCode> method.

<CodeWithHeader title="SSO_ORGANIZATION_ENABLED">
...
window.onmessage = event => {
  if (
    ...
    event.data &&
    event.data.event_type === 'ORGANIZATION_SSO_ENABLED' // Check specific event type
  ) {
