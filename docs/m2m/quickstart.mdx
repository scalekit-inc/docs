...
def create_m2m_client(organization_id: str) -> tuple[OrganizationClient, str]:
    """
    Creates a new M2M client for the specified organization.

    Args:
        organization_id: The ID of the organization to create the client for

    Returns:
        A tuple containing the created client object and the client secret
    """
    # Initialize Faker for generating test data
    faker = Faker()

    # Create M2M client configuration
    m2m_client = OrganizationClient(
        name=faker.company(),
        description=faker.sentence(),
        custom_claims=[{
            "key": "wksp_id",
            "value": f"wks_{faker.credit_card_number()}"
        }],
        scopes=["write", "read"],
        audience=["my-own-api"],
        expiry=3600
    ...
    # Create the client and get response
    ...
    return response[0].client, response[0].plain_secret
```
</TabItem>
</Tabs>
</CodeWithHeader>
...
def authenticate_m2m_client(organization_id: str) -> dict:
    """
    Creates and authenticates an M2M client, returning the access token.

    Args:
        organization_id: The ID of the organization to create the client for

    Returns:
        A dictionary containing the access token and related information
    """
    # Create the M2M client
    client, client_secret = create_m2m_client(organization_id)
