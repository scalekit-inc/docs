...
|-----------|------|----------|-------------|
| `name` | string | Optional | A descriptive name for the M2M client (e.g., "GitHub Actions Deployment Service") |
| `description` | string | Optional | A detailed explanation of the client's purpose and usage |
| `custom_claims` | array | Optional | Key-value pairs that provide additional context about the client. Each claim must have a `key` and `value` field |
| `scopes` | array | Optional | List of permissions the client needs (e.g., ["deploy:applications", "read:deployments"]) |
| `audience` | array | Optional | List of API endpoints this client will access (e.g., ["deployment-api.acmecorp.com"]) |
| `expiry` | number | Optional | Token expiration time in seconds. Defaults to 3600 (1 hour) |

The API response contains the client details, including the `client_id` and `client_secret`.

<CodeWithHeader title="Register an M2M client response">
