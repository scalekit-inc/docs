# AWS Cognito
<Subtitle>AWS Cognito Integration with Scalekit for Single Sign-On (SSO)</Subtitle>
...
:::info
Wondering which OIDC client library to use? Refer to: **AWS Cognito > Applications > App clients > Quick setup guide**. This section provides recommended libraries and setup instructions.
:::
...
Scalekit integrates seamlessly with [major identity providers](/integrations). Use Scalekit's [admin portal](/sso/quickstart-admin-portal) to onboard enterprise customers, enabling them to set up connections between their identity providers and your application. M
