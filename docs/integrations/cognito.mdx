...
For a practical demonstration, refer to the example application showcasing this integration.
...
| **Issuer URL**        | The URL of the OIDC issuer, this is your Environment URL in Scalekit > API Config (e.g., <SimpleCode>https:<br/>//subdomain.scalekit.dev</SimpleCode>).|
...
:::info
The example code above uses the `openid-client` library for node.js. See Quick setup guides in your AWS Cognito dashboard (Applications > App clients > Quick setup guide) for other libraries for Python, Java and Go.
:::
