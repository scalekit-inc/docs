...
- Follow the Custom Domains steps in the **Customization > Custom Domain** section in your Dashboard.
- Add a CNAME record in your domain’s DNS settings. For example, if your project’s default domain is <SimpleCode>https:<span></span>//yourapp.scalekit.com</SimpleCode>, then:
  ...
  - Verify that no extra domain names are appended by your DNS registrar. If extra parts are added (e.g., <SimpleCode>auth.yourapp.com.yourapp.com</SimpleCode>), remove the redundant portion by using only the appropriate prefix (e.g., <SimpleCode>auth</SimpleCode>).
