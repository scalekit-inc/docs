...
var organizations = await scalekitClient.Organization.ListOrganizations(10);
Console.WriteLine($"Name of the organization: {organizations.Organizations[0].DisplayName}");
```
...
```csharp showLineNumbers
// Get directory using organization ID and directory ID
var directory = await scalekitClient.Directory.GetDirectory(organizationId, directoryId);

// Get directory using organization ID
var primaryDirectory = await scalekitClient.Directory.GetPrimaryDirectoryByOrganizationId(organizationId);
```
...
var usersResponse = await scalekitClient.Directory.ListDirectoryUsers(organizationId, directoryId, options);
// usersResponse.Users[0].Email has the email of the first user in the directory
```
...
var groupsResponse = await scalekitClient.Directory.ListDirectoryGroups(organizationId, directoryId, options);
```
